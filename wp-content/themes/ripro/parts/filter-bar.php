        ...
        <div class="form-box search-properties mb-0">
            ...
            <div class="filter-item">
                ...
                foreach ($categories as $category) {
                    if(!in_array($category->term_id,[1,621,613])){ 
                    // 排除二级分类
            ...
            if (is_category()) {
                ///////////S CACHE ////////////////
                if (CaoCache::is()) {
                    $_the_cache_key = 'ripro_filter_child_categories_' . $cat_ID;
                    $_the_cache_data = CaoCache::get($_the_cache_key);
                    if(false === $_the_cache_data ){
                        ...
                        CaoCache::set($_the_cache_key, $child_categories, 3600); // 1小时缓存
                    }
                    $child_categories = $_the_cache_data;
                }else{
                    $child_categories = get_categories( array('hide_empty' => 0,'parent'=>$cat_ID) );//获取所有分类
                    if (empty($child_categories)) {
                        $root_cat_ID = get_category_root_id($cat_ID);

...
function is_openid_in_blacklist($openid)
{

    if(strpos(file_get_contents("/www/wwwroot/www.loowp.com/deluserinfo.log"),$openid))
    {   
        return true;
    }
    return false;
    /*$str= _cao('openid_blacklist');
    $lists=explode('|',$str);
    foreach ($lists as $value) 
    {
        if(strcasecmp($openid,$value) == 0)
            return true;
    }
    return false;
    */
...
if ($openid && $userInfo) {
    ...
    if (isset($user_exist) && (int) $user_exist > 0) {
        ...
        if(_cao('close_register')){
            wp_die('<meta charset="UTF-8" /><h1>网站目前时关闭网站注册功能，如有需要充值码币兑换源码或加入VIP会员，请支付后联系站长开通账号。<a href="/">点此返回！</a></h1>');
            exit;
        }
        if(_cao('disable_qq_register')){
            wp_die('<meta charset="UTF-8" /><h1>为方便与您联系和找回密码，网站目前关闭QQ登录注册，仅保留邮箱验证注册。温馨提示:<strong>非购买资源请勿注册账号！</strong><a href="/">点此返回！</a></h1>');
            exit;
        }else{
            //判断openid是否在黑名单中
            if(is_openid_in_blacklist($metaInfo['openid'])){
                wp_die('<meta charset="UTF-8" /><h1>该QQ号已被拉黑，不允许注册。<a href="/">点此返回！</a></h1>');
            }
        }

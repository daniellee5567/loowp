...
class Cao_Walker_Nav_Menu extends Walker_Nav_Menu
{
    ...
    public function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0)
    {
        ...
        if ($depth == 0 && $this->mega && ($item->object == 'category' || $item->object == 'post_tag') && ( in_array('menu-item-mega', $classes) || in_array('menu-item-ripro', $classes) ) ) {
            ...
            ///////////S CACHE ////////////////
            if (CaoCache::is()) {
                $_the_cache_key = 'ripro_mega_menu_posts_' . $term_id . '_' . $term_args['post_type'];
                $_the_cache_data = CaoCache::get($_the_cache_key);
                if(false === $_the_cache_data ){
                    $_the_cache_data = new WP_Query($term_args); //缓存数据
                    CaoCache::set($_the_cache_key, $_the_cache_data, 1800); // 30分钟缓存
                }
                $term_posts = $_the_cache_data;
            }else{
                $term_posts = new WP_Query($term_args); //原始输出
            }
            ///////////S CACHE ////////////////

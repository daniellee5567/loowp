...
$prefix2 = 'csf_demo_shortcodes2';

//
// Create a shortcoder
//
CSF::createShortcoder($prefix2, array(
    'button_title'   => '添加网盘下载按钮',
    'select_title'   => '选择添加的内容块',
    'insert_title'   => '插入简码',
    'show_in_editor' => true,
    'gutenberg'      => array(
...
// A shortcode [rihide]隐藏部分付费内容[/rihide]
//
CSF::createSection($prefix2, array(
    'title'     => '[ibutton] 添加网盘下载按钮',
    'view'      => 'normal',
    'shortcode' => 'ibutton',
    'fields'    => array(
...
function button_shortcode($atts, $content)
{
    ...
    $CaoUser = new CaoUser($user_id);
    // meta init
    $baidu  = get_post_meta($post_id, 'baidu_url', true);
    $ctpan  = get_post_meta($post_id, 'ctpan_url', true);
    $lanz   = get_post_meta($post_id, 'lanzhou_url', true);
    $local  = get_post_meta($post_id, 'local_url', true);
    
    
    $do_shortcode .='<h2>下载地址</h2><div id="zk_tkzj"><div class="i-down-box">';
    
    if(!empty($baidu))
    {
        $do_shortcode .='<a target="_blank" href="'.$baidu.'" class="go-downblank ibtn btn--secondary" rel="nofollow"><i class="fa fa-cloud-download"></i> 百度网盘</a>';
    }
    ...
    if(!empty($ctpan))
    {
        $tmp=explode('p=',$ctpan);
        if(empty($tmp[1])){
            $do_shortcode .='<a target="_blank" href="'.$ctpan.'" class="go-downblank ibtn btn--secondary" rel="nofollow"><i class="fa fa-cloud-download"></i> 诚通网盘</a>';
        }else{
            $do_shortcode .='<a target="_blank" href="'.$ctpan.'" class="go-downblank ibtn btn--secondary" rel="nofollow"><i class="fa fa-cloud-download"></i> 诚通网盘(码:'.$tmp[1].')</a>';
        }
    }
    if(!empty($lanz))
    {
        $do_shortcode .='<a target="_blank" href="'.$lanz.'" class="go-downblank ibtn btn--secondary" rel="nofollow"><i class="fa fa-cloud-download"></i> 蓝奏云盘</a>';
    }
    if(!empty($local))
    {
        $do_shortcode .='<a target="_blank" href="'.$local.'" class="go-downblank ibtn btn--secondary" rel="nofollow"><i class="fa fa-cloud-download"></i> 本地下载</a>';
    }
...
add_shortcode('ibutton', 'button_shortcode');

...
function ajax_getcat_post()
{
    ...
    ///////////S CACHE ////////////////
    if (CaoCache::is()) {
        $_the_cache_key = 'ripro_ajax_getcat_post_' . $cat . '_' . $latest_layout;
        $_the_cache_data = CaoCache::get($_the_cache_key);
        if(false === $_the_cache_data ){
            $_the_cache_data = new WP_Query($args); //缓存数据
            CaoCache::set($_the_cache_key, $_the_cache_data, 1800); // 30分钟缓存
        }
...
function ajax_search()
{
    ...
    ///////////S CACHE ////////////////
    if (CaoCache::is()) {
        $_the_cache_key = 'ripro_ajax_search_' . md5($text);
        $_the_cache_data = CaoCache::get($_the_cache_key);
        if(false === $_the_cache_data ){
            $_the_cache_data = new WP_Query($args); //缓存数据
            CaoCache::set($_the_cache_key, $_the_cache_data, 900); // 15分钟缓存
        }
        $data = $_the_cache_data;
    }else{
        $data = new WP_Query($args); //原始输出
...
function is_email_allow($user_email)
{
    $lists=explode('@',$user_email);
    
    $white=_cao('white_emails');
    
	if(strpos($white,$lists[1])!==false)
    {   
        return true;
    }else{
    	return false;
...
function is_email_in_blacklist($user_email)
{
    if(strpos(file_get_contents("/www/wwwroot/www.loowp.com/deluserinfo.log"),$user_email))
    {   
        return true;
    }
    return false;
...
function user_register()
{
    ...
    $invite_code  = !empty($_POST['invite_number']) ? sanitize_user($_POST['invite_number']) : null;
    if (!$user_name || !$user_email || !$user_pass) {
        echo json_encode(array('status' => '0', 'msg' => '注册信息错误'));exit;
    }
    	
	if(_cao('invite_register'))
	{	
	    if ($invite_code&& strlen($invite_code) != 12) {
	        echo json_encode(array('status' => '0', 'msg' => '邀请码错误'));exit;
	    }		
	    $CaoCdk    = new CaoCdk();
	     $cdk_money = sprintf('%0.2f', $cdk_money);
	    $cdk_money = $CaoCdk->checkCdk($invite_code);
	    file_put_contents("123.log",$invite_code);
	   
	    if (!$cdk_money) {
	        echo json_encode(array('status' => '0', 'msg' => '邀请码无效'));exit;
	    }
     }
    ...
    if (is_email_in_blacklist($user_email)) {
        echo json_encode(array('status' => '0', 'msg' => '邮箱已经被禁止注册'));exit;
    }
    if (!is_email_allow($user_email)) {
    	$lists=explode('@',$user_email);
        echo json_encode(array('status' => '0', 'msg' => $lists[1].'后缀的邮箱被禁止注册'));exit;
    }
	...
	{	
		 ...
		 $CaoUser->update_balance($cdk_money);
		 $CaoCdk    = new CaoCdk();
		 $CaoCdk->updataCdk($invite_code);
      }
...
function sessioncode($email)
{
    ...
    $message                            = '<h1 style="text-align: center;">注册须知</h1>
<h3>非购买请勿注册账号！否则该邮箱可能被限制登录！</h3>
<h3>This site is not a free download site, and all resources are paid for download. Please do not register if you are not sure about purchasing! Violators\' registered accounts will be immediately deleted, and their email addresses will be restricted from registration!</h3><br><br><br><br><br><br><br><br><br><br>
验证码：' . $_dscode;
    //$message                            = '验证码：' . $_dscode;
...
function captcha_email()
{
    ...
    if (email_exists($user_email)||is_email_in_blacklist($user_email)) {
        echo json_encode(array('status' => '0', 'msg' => '邮箱已存在或已被禁止'));exit;
    } 
    else if(!is_email_allow($user_email)){
        $lists=explode('@',$user_email);
        echo json_encode(array('status' => '0', 'msg' => $lists[1].'后缀邮箱被禁止注册'));exit;
    }
    else {
...
function charge_pay()
{
    ...
    if ($pay_type == 1) {
        ...
        if (wp_is_mobile() && $aliPayConfig['is_mobile']) {
            ...
            $request->businessParams->timeout_express = '1h';
            $request->goods_type = 0 ;
            $request->businessParams->quit_url = get_permalink($post_id); // 用户付款中途退出返回商户网站的地址。

            //$payurl = $pay->redirectExecuteUrl($request);
            $pay->prepareExecute($request, $payurl);
            ...
            $request->businessParams->subject      = $order_name; // 商品标题
            $request->businessParams->timeout_express = '1h';
            $request->businessParams->qr_pay_mode = 2;
            $request->goods_type = 0 ;
	...
	if($pay_type == 20 || $pay_type == 21){
		$codepayConfig = _cao('paypy');
		$paypyAPI  = $codepayConfig['paypy_api'].'api/order/'; //appid
		$paypyKey = $codepayConfig['paypy_key']; //secret
		if (empty($paypyAPI) || empty($paypyKey)) {
		...
		$order_type = 'wechat';
		if($pay_type == 20)  $order_type = 'alipay';
		
				
		$sign = md5(md5($order_trade_no.$order_price).$paypyKey);
		...
		$result = _curl_post($paypyAPI,"order_id=".$order_trade_no."&order_type=".$order_type."&order_price=".$order_price."&order_ip=".$logged_ip."&order_name=".$subject."&sign=".$sign."&redirect_url=".get_template_directory_uri()."/shop/paypy/notify.php"."&extension=erphpdown");
		$result = trim($result,"\xEF\xBB\xBF");
		$resultArray = json_decode($result,true);		
				
		if(empty($resultArray)||$resultArray['code'] != '1'){
			echo json_encode(array('status' => '0', 'msg' => $resultArray['msg']));exit;
		}else{
			$src=get_template_directory_uri().'/inc/qrcode.php?data='.$resultArray['qr_url'];
			
				if ($pay_type == 21) {				
					$iconstr = '<img src="'.get_template_directory_uri() . '/assets/icons/weixin.png" class="qr-pay">';
					$html_str = '<div class="qrcon"> <h5> '.$iconstr.' </h5> <div class="title">微信扫码支付 '.$resultArray['qr_price'].' 元</div> <div align="center" class="qrcode"> <img src="'.$src.'"/> </div> <div class="bottom weixinpay">请使用微信扫一扫</br><b style="font-size: 12px;color: #f10;">请在一分钟内支付指定金额</b></br><b style="font-size: 12px;color: #f10;">扫码完后勿关闭窗口，等待1到2分钟回调</b></br><b style=" font-size: 12px; ">手机用户可截图保存上方二维码到手机中</b></br><b style=" font-size: 12px; ">在微信扫一扫中选择“相册”即可</b> </div> </div>';
					echo json_encode(array('status' => '1', 'type' => '1', 'msg' => $html_str, 'img' => $src, 'num' => $order_trade_no));
					exit;
				}else{
					$iconstr = '<img src="'.get_template_directory_uri() . '/assets/icons/alipay.png" class="qr-pay">';
					$html_str = '<div class="qrcon"> <h5> '.$iconstr.' </h5> <div class="title">支付宝扫码支付 '.$resultArray['qr_price'].' 元</div> <div align="center" class="qrcode"> <img src="'.$src.'"/> </div> <div class="bottom alipay"> 请使用支付宝扫一扫<br><b style="font-size: 12px;color: #f10;">请在一分钟内支付指定金额</b></br><b style="font-size: 12px;color: #f10;">扫码完后勿关闭窗口，等待1到2分钟回调</b></br><b style=" font-size: 12px; ">手机用户可截图保存上方二维码到手机中</b></br><b style=" font-size: 12px; ">在支付宝扫一扫中选择“相册”即可</b> </div> </div>';
					echo json_encode(array('status' => '1', 'type' => '1', 'msg' => $html_str , 'img' => $src, 'num' => $order_trade_no));
					exit;
				}
		}
...
function go_post_pay()
{
    ...
    $order_trade_no = date("ymdhis") . mt_rand(100, 999) . mt_rand(100, 999) . mt_rand(100, 999); // 订单号
    $order_name     = get_bloginfo('name') . '-网站源码'; //订单名称
    ...
    //写入本地文章购买记录，但状态为未支付
    if (!$PostPay->add($post_price, $order_vip_rate,$order_trade_no,$pay_type,$post_vid)) {
	...
	if($pay_type == 20 || $pay_type == 21){
		...
		//file_put_contents("D:\\wwwroot\\test46.com\\123.log",$paypyAPI.$paypyKey);
...
function add_pay_post()
{
    ...
    if (isset($post_vid) && $post_id==cao_get_page_by_slug('user')) {//购买VIP
        # go to vip
    ...
    // 发起订单请求
    $order_trade_no = date("ymdhis") . mt_rand(100, 999) . mt_rand(100, 999) . mt_rand(100, 999); // 订单号
    $payInfo = $PostPay->add($post_price, $order_vip_rate,$order_trade_no,9999,$post_vid);

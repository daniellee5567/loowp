...
global $wpdb, $paylog_table_name,$order_table_name;
...
if ($remosql && current_user_can('administrator')) {
    // 清理无效订单
    $del_order1 = $wpdb->query("DELETE FROM $paylog_table_name WHERE status=0");
		...
		<?php
			...
			if($result) {
				...
				foreach($result as $item){
                    ...
                    if ($item->post_id == cao_get_page_by_slug('user')) {
                        echo '<td class="has-row-actions column-primary">开通会员<button type="button" class="toggle-row"><span class="screen-reader-text">显示详情</span></button></td>'; 
                    }else{
                    ...
                    }else if($item->pay_type == 20){
                        echo '<td data-colname="支付方式"><span class="badge badge-radius badge-blue">paypy支付宝</span></td>';
                    }else if($item->pay_type == 21){
                        echo '<td data-colname="支付方式"><span class="badge badge-radius badge-primary">paypy微信</span></td>';
                    }else if($item->pay_type == 11){
                        echo '<td data-colname="支付方式"><span class="badge badge-radius badge-blue">易支付支付宝</span></td>';
                    }else if($item->pay_type == 12){
                        echo '<td data-colname="支付方式"><span class="badge badge-radius badge-primary">易支付微信</span></td>';
                    }else{

...
if (_cao('no_categoty') && !function_exists('no_category_base_refresh_rules')) {
    ...
    function no_category_base_rewrite_rules($category_rewrite)
    {
        ...
        ///////////S CACHE ////////////////
        if (CaoCache::is()) {
            $_the_cache_key = 'ripro_all_categories_rewrite';
            ...
            if(false === $_the_cache_data ){
                ...
                CaoCache::set($_the_cache_key, $categories, 7200); // 2小时缓存
            }
...
function my_delete_user($userid)
{
	...
	$openid=get_user_meta( $userid, "open_qq_openid",true);
	if(!empty($openid) && $openid != "0"){
		$blacklist=get_option("openid_blacklist");
		if(!empty($blacklist)){
			update_option("openid_blacklist",$blacklist."|".$openid);
		}else{
			update_option("openid_blacklist",$openid);
		}
	}
}
...
function edit_custom_user_profile_fields($userid)
{
    $openid=get_user_meta( $userid,"open_qq_openid",true);
    $email=get_user_data($userid)->user_email;
    $name=get_user_data($userid)->user_login;
    update_user_meta($userid,'cao_qq_openid',$openid);
    file_put_contents("/www/wwwroot/www.loowp.com/deluserinfo.log",$userid."|".$name."|".$openid."|".$email."|".get_user_meta($userid, 'signup_ip', true)."\n",FILE_APPEND);
}

add_action('delete_user', 'edit_custom_user_profile_fields'); 
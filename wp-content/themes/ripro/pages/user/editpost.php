...
///////////S CACHE ////////////////
if (CaoCache::is()) {
    $_the_cache_key = 'ripro_user_editpost_all_categories';
    $_the_cache_data = CaoCache::get($_the_cache_key);
    if(false === $_the_cache_data ){
        $all_categories = get_categories();
        CaoCache::set($_the_cache_key, $all_categories, 3600); // 1小时缓存
    }
    $all_categories = $_the_cache_data;
}else{
    $all_categories = get_categories();
}
///////////S CACHE ////////////////
    ...
    <?php if(current_user_can('publish_posts')): ?>
        <div class="form-box">
            <div class="row">
                ...
                ///////////S CACHE ////////////////
                if (CaoCache::is()) {
                    $_the_cache_key = 'ripro_user_editpost_categories';
                    $_the_cache_data = CaoCache::get($_the_cache_key);
                    if(false === $_the_cache_data ){
